...
const getTrendingTopics = unstable_cache(
    async()=>{
        const result = await prisma.$queryRaw<{hastag :string; count: bigint}[]>`
        SELECT LOWER(unnest(regexp_matches(content, '#[[:alnum:]_]+', 'g'))) AS hastag, COUNT(*) AS count
        FROM posts
        GROUP BY (hastag)
        ORDER BY count DESC, Hastag ASC
        LIMIT 5
        `;
